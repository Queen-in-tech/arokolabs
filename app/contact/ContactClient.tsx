              ...
              {[
                ...
                <motion.div key={item.title} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="flex gap-4">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10">
                    ...
                    <div>
                      <p className="font-display text-sm font-semibold text-[--color-foreground]">{item.title}</p>
                      <p className="font-body text-sm text-[--color-muted-foreground]">{item.desc}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
